...
from .configuration_MoTSE import MoTSEConfig
...
class MoTSEAttention(nn.Module):
    ...
    def __init__(self, config: MoTSEConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
TIME_MOE_ATTENTION_CLASSES = {
    "eager": MoTSEAttention,
    'flash_attention_2': MoTSEFlashAttention2,
}
...
class MoTSEDecoderLayer(nn.Module):
    def __init__(self, config: MoTSEConfig, layer_idx: int):
        ...
        if self.config.use_dense:
            self.ffn_layer = MoTSEMLP(
                hidden_size=self.config.hidden_size,
                intermediate_size=self.config.intermediate_size,
                hidden_act=self.config.hidden_act,
            )
        else:
            self.ffn_layer = MoTSESparseExpertsLayer(config)
        self.input_layernorm = MoTSERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = MoTSERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class MoTSEPreTrainedModel(PreTrainedModel):
    config_class = MoTSEConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["MoTSEDecoderLayer"]
...
class MoTSEModel(MoTSEPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`MoTSEDecoderLayer`]
    ...
    def __init__(self, config: MoTSEConfig):
        ...
        self.norm = MoTSERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class MoTSEForPrediction(MoTSEPreTrainedModel, TSGenerationMixin):
