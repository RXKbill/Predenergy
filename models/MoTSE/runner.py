...
from MoTSE.datasets.MoTSE_dataset import MoTSEDataset
from MoTSE.datasets.MoTSE_window_dataset import MoTSEWindowDataset
from MoTSE.models.modeling_MoTSE import MoTSEForPrediction, MoTSEConfig
from MoTSE.trainer.hf_trainer import MoTSETrainingArguments, MoTSETrainer
from MoTSE.utils.dist_util import get_world_size
from MoTSE.utils.log_util import logger, log_in_local_rank_0
...
class MoTSERunner:
    ...
    def load_model(self, model_path: str = None, from_scatch: bool = False, **kwargs):
        ...
        if from_scatch:
            config = MoTSEConfig.from_pretrained(model_path, _attn_implementation=attn)
            model = MoTSEForPrediction(config)
        else:
            model = MoTSEForPrediction.from_pretrained(model_path, **kwargs)
        return model
    ...
    def train_model(self, from_scratch: bool = False, **kwargs):
        ...
        training_args = MoTSETrainingArguments(
    ...
    def get_train_dataset(self, data_path, max_length, stride, normalization_method):
        ...
        dataset = MoTSEDataset(data_path, normalization_method=normalization_method)
        log_in_local_rank_0('Processing dataset to fixed-size sub-sequences...')
        window_dataset = MoTSEWindowDataset(dataset, context_length=max_length, prediction_length=0, stride=stride, shuffle=False)
