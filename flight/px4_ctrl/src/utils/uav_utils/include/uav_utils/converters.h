                      ...
                      Eigen::Vector3d& v, Eigen::Quaterniond& q)
{
    ...
    // Eigen::Quaterniond quat_correction(0, 1, 0, 0);
    // Eigen::Quaterniond quat_corrected = quat_correction * quat;

    // Eigen::Vector3d euler_angles = quat_corrected.toRotationMatrix().eulerAngles(0, 1, 2);

    // euler_angles(1) = -euler_angles(1);
    // euler_angles(2) = -euler_angles(2);

    // Eigen::Quaterniond quat_final;

    // quat_final = Eigen::AngleAxisd(euler_angles[0], Eigen::Vector3d::UnitX()) *
