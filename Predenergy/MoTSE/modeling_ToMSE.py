...
from .configuration_time_moe import ToMSEConfig
...
class ToMSEInputEmbedding(nn.Module):
    """
    Use a mlp layer to embedding the time-series.
    """

    def __init__(self, config: ToMSEConfig):
        super().__init__()
        self.config = config
...
# Copied from transformers.models.mistral.modeling_mistral.MistralRotaryEmbedding with Mistral->ToMSE
class ToMSERotaryEmbedding(torch.nn.Module):
...
# Copied from transformers.models.llama.modeling_llama.LlamaRMSNorm with Llama->ToMSE
class ToMSERMSNorm(torch.nn.Module):
...
class ToMSETemporalBlock(nn.Module):
...
class ToMSEMLP(ToMSETemporalBlock):
    def __init__(self, hidden_size: int, intermediate_size: int, hidden_act: str):
...
class ToMSESparseExpertsLayer(nn.Module):
    def __init__(self, config):
        ...
        self.shared_expert = ToMSETemporalBlock(
            hidden_size=self.config.hidden_size,
...
# Copied from transformers.models.qwen2.modeling_qwen2.Qwen2Attention with Qwen2->ToMSE
class ToMSEAttention(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper. Modified to use sliding window attention: Longformer
    and "Generating Long Sequences with Sparse Transformers".
    """

    def __init__(self, config: ToMSEConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        self.rotary_emb = ToMSERotaryEmbedding(
            self.head_dim,
...
class ToMSEFlashAttention2(ToMSEAttention):
...
TIME_MOE_ATTENTION_CLASSES = {
    "eager": ToMSEAttention,
    'flash_attention_2': ToMSEFlashAttention2,
}


class ToMSEDecoderLayer(nn.Module):
    def __init__(self, config: ToMSEConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        ...
        if self.config.use_dense:
            self.ffn_layer = ToMSEMLP(
                hidden_size=self.config.hidden_size,
                intermediate_size=self.config.intermediate_size,
                hidden_act=self.config.hidden_act,
            )
        else:
            self.ffn_layer = ToMSESparseExpertsLayer(config)
        self.input_layernorm = ToMSERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = ToMSERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class ToMSEPreTrainedModel(PreTrainedModel):
    config_class = ToMSEConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["ToMSEDecoderLayer"]
...
class ToMSEModel(ToMSEPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`ToMSEDecoderLayer`]
    ...
    def __init__(self, config: ToMSEConfig):
        ...
        self.embed_layer = ToMSEInputEmbedding(config)
        self.layers = nn.ModuleList(
            [ToMSEDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self._attn_implementation = config._attn_implementation
        self.norm = ToMSERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class ToMSEOutputLayer(nn.Module):
...
class ToMSEForPrediction(ToMSEPreTrainedModel, TSGenerationMixin):
